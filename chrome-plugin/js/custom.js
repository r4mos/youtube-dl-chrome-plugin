$(document).ready(function () {
  ...
  $.fn.disable = function (disabled) {
    if (this.disabled !== undefined) this.disabled = disabled;
    else disabled ? this.attr('disabled', '') : this.removeAttr('disabled');
  };
  ...
  $.get('version.txt', function (version) {
    $.getJSON(getAPIURL() + '/version', function (data) {
      ...
      .always(function () {
        ...
        $('#main').show();
        $.get(
          'https://raw.githubusercontent.com/r4mos/youtube-dl-chrome-plugin/master/chrome-plugin/version.txt',
...
function loadGet() {
  $.getJSON(getAPIURL() + '/version', function (data) {
    $.each(data, function (key, val) {
      if (key == 'youtube-dl') {
        $('#getLegend').append(
          ' <span class="badge remove">' + val + '</span>'
        );
      }
    });
    loadGetContent();
...
function loadGetContent() {
  $.getJSON(getAPIURL() + '/info?url=' + $('#vidoeUrl').val(), function (data) {
    ...
    html += '  <p>';
    html +=
      '    <a href="' + prefix + data.info.url + '" class="btn btn-primary">';
    html +=
    ...
    html +=
      ...
      prefix +
      data.info.url +
      '" class="btn btn-primary" download id="downloadPrimary">';
    html +=
    ...
    if (data.info.formats != null) {
      html += '<hr/>';
      html +=
        '<table id="tableMore" class="table table-striped table-hover hide">';
      html += '  <thead>';
      html += '    <tr>';
      html +=
        '      <th><span class="glyphicon glyphicon-resize-full" aria-hidden="true"></span> Extension</th>';
      html +=
        '      <th><span class="glyphicon glyphicon-paperclip" aria-hidden="true"></span> Format</th>';
      html +=
        '      <th><span class="glyphicon glyphicon-link" aria-hidden="true"></span> Links</th>';
      html += '    </tr>';
      html += '  </thead>';
      html += '  <tbody>';
      $.each(data.info.formats, function (key, val) {
        ...
        html += '    <td>';
        html +=
          '    <a href="' + prefix + val.url + '" class="btn btn-default">';
        html +=
          '      <span class="glyphicon glyphicon-play" aria-hidden="true"></span>';
        html += '      Play';
        html += '    </a>';
    ...
    html += '</div>';
    $('#get').append(html);
    const downloadPrimary = $('#downloadPrimary');
    downloadPrimary.on('click', (ev) => {
      ev.preventDefault();
      /**
       * @type {HTMLLinkElement}
       */
      const el = ev.currentTarget;
      if (downloadPrimary.attr('disabled')) return;
      downloadPrimary.disable(true);
      return downloadFileDirect(data.info.url, data.info)
        .then(() => {
          downloadPrimary.disable(false);
        })
        .catch(() => {
          downloadPrimary.disable(false);
        });
...
function downloadFileDirect(url, info) {
  const filename = info.title + '.' + info.ext;
  return fetch(url)
    .then((resp) => resp.blob())
    .then((blob) => {
